...
FROM `order` o
LEFT JOIN (
    SELECT order_id, SUM(amount) refunded_amount FROM refund WHERE status = 3 GROUP BY order_id
) r ON r.order_id = o.id
WHERE o.pay_status = 1 AND o.order_status IN (1, 2, 3)
GROUP BY o.user_id;

INSERT INTO daily_business_stats
    (stat_date, order_count, paid_order_count, paid_user_count, sales_amount, refund_count, refund_amount, visitor_count, new_user_count)
SELECT d.stat_date,
       COALESCE(a.order_count, 0), COALESCE(o.paid_order_count, 0), COALESCE(o.paid_user_count, 0), COALESCE(o.sales_amount, 0),
       COALESCE(r.refund_count, 0), COALESCE(r.refund_amount, 0), COALESCE(b.visitor_count, 0), COALESCE(u.new_user_count, 0)
FROM (
    SELECT DATE(create_time) AS stat_date FROM `order`
    UNION SELECT DATE(create_time) FROM refund
    UNION SELECT DATE(browse_time) FROM browse_history
    UNION SELECT DATE(create_time) FROM `user`
) d
LEFT JOIN (
    SELECT DATE(create_time) stat_date, COUNT(*) order_count FROM `order` GROUP BY DATE(create_time)
